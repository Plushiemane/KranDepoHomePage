...
const DeseryPage: React.FC = () => {
  ...
  // Filter items that are desserts
  const desery = menuItems.filter(item => 
    item.nazwa.toLowerCase().includes('deser') || 
    item.nazwa.toLowerCase().includes('lody') ||
    item.nazwa.toLowerCase().includes('ciasto') ||
    item.nazwa.toLowerCase().includes('ciastko') ||
    item.nazwa.toLowerCase().includes('słodkie')
  );
