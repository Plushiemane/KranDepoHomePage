...
const ZupyPage: React.FC = () => {
  ...
  // Filter items that include "zupa" in their name (case-insensitive)
  const zupy = menuItems.filter(item => 
    item.nazwa.toLowerCase().includes('zupa') || 
    item.nazwa.toLowerCase().includes('żurek') || 
    item.nazwa.toLowerCase().includes('flaki') ||
    item.nazwa.toLowerCase().includes('barszcz') ||
    item.nazwa.toLowerCase().includes('rosół')
  );
